...
const cache = new Map(); // days → { ts, payload }
...
async function buildPayload(days) {
  const dates = rangeDates(days);
  const startDate = dates[0];

  const [adds, ...callsPerCampaign] = await Promise.all([
    fetchAdds(startDate),
    ...CAMPAIGNS.map(c => fetchCampaignCalls(c.id, startDate)),
  ]);
...
async function getPayload(days) {
  ...
  cache.set(days, { ts: Date.now(), payload });
  return payload;
}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="note">Successful adds logged by the three workflows (log starts July 10, 2026)</div>

/**
 * Per-workflow occurred / added / called rollups for the dashboard.
 *
 * "Occurred" = the eligible population per workflow, by day (UTC):
 ...
 *
 * "Added"  = rows in the "SDR Adds Log" sheet tab (complete from July 9, 2026).
 * "Called" = SalesDialer calls per campaign (dials + unique contact numbers).
 */
...
function bucketDefs(now = new Date()) {
  const day = (d) => d.toISOString().slice(0, 10);
  const today = day(now);
  const yesterday = day(new Date(now.getTime() - 86400000));
  const last7Start = day(new Date(now.getTime() - 6 * 86400000));
  const y = now.getUTCFullYear(), m = now.getUTCMonth();
  const thisMonthStart = day(new Date(Date.UTC(y, m, 1)));
  const lastMonthStart = day(new Date(Date.UTC(y, m - 1, 1)));
  const lastMonthEnd = day(new Date(Date.UTC(y, m, 0)));
  return [
    { key: 'today',     label: 'Today',       from: today,          to: today },
    { key: 'yesterday', label: 'Yesterday',   from: yesterday,      to: yesterday },
    { key: 'last7',     label: 'Last 7 days', from: last7Start,     to: today },
...
function sumRange(byDay, from, to) {
  let n = 0;
  for (const [d, v] of Object.entries(byDay)) if (d >= from && d <= to) n += v;
  return n;
}
...
async function noShowsByDay(fromDate) {
  ...
  try {
    const { rows } = await pg.query(`
      ...
      SELECT demo_scheduled_date::date AS day, COUNT(*)::int AS n
      FROM latest_rows
      WHERE show_status = 'N'
        AND demo_scheduled_date >= $1
...
async function formLeadsNoBookingByDay(fromDate) {
  ...
  try {
    const { rows } = await pg.query(`
      SELECT gfl.created_at::date AS day, COUNT(DISTINCT LOWER(TRIM(gfl.email)))::int AS n
      FROM gw_form_leads gfl
      WHERE gfl.created_at >= $1
...
async function leadformNoBookingByDay(fromDate) {
  ...
  const bookedIdx = headers.indexOf('Demos Booked');
  const byDay = {};
  for (const row of rows.slice(1)) {
    const day = String(row[tsIdx] || '').slice(0, 10);
    if (!day || day < fromDate) continue;
    if (String(row[bookedIdx] || '').trim().toLowerCase() === 'yes') continue;
...
/**
 * @param addsByDayByCampaign  day → campaignId → count   (from the adds log)
 * @param callsPerCampaign     campaignId → raw call objects (window ≥ last month start)
 */
async function buildSummary(addsByDayByCampaign, callsPerCampaign) {
  const buckets = bucketDefs();
  const fromDate = buckets.find(b => b.key === 'lastMonth').from;
  ...
  const occurredBy = { '3190746': noShow, '3190752': noBooking, '3309032': estimator };

  const addsBy = {}; // campaignId → day → n
  for (const [day, per] of Object.entries(addsByDayByCampaign)) {
    for (const [cid, n] of Object.entries(per)) ((addsBy[cid] ||= {})[day] = ((addsBy[cid] || {})[day] || 0) + n);
  }

  const summary = {};
  for (const [cid, calls] of Object.entries(callsPerCampaign)) {
    const callsByDay = {};
    const uniqueByDay = {}; // day → Set
    for (const c of calls) {
      const d = c.call_date;
      callsByDay[d] = (callsByDay[d] || 0) + 1;
      (uniqueByDay[d] ||= new Set()).add(c.contact_number || c.contact_id);
    }
    summary[cid] = buckets.map(b => {
      const unique = new Set();
      for (const [d, set] of Object.entries(uniqueByDay)) {
        if (d >= b.from && d <= b.to) for (const v of set) unique.add(v);
      }
      return {
        key: b.key,
        label: b.label,
        occurred: sumRange(occurredBy[cid] || {}, b.from, b.to),
        added: sumRange(addsBy[cid] || {}, b.from, b.to),
        calls: sumRange(callsByDay, b.from, b.to),
        uniqueCalled: unique.size,
      };
    });
  }
  return { buckets: buckets.map(b => ({ key: b.key, label: b.label })), byCampaign: summary };
}

module.exports = { buildSummary, bucketDefs };
